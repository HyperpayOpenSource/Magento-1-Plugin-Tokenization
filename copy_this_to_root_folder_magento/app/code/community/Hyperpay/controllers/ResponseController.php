...
class Hyperpay_ResponseController extends Mage_Core_Controller_Front_Action
{
   ...
   protected function _getCheckout()
    ...
    private function _getPostResponseSadadActionUrl($order)
    {
        ...
        {
            Mage::throwException('Merchant Reference Number is not found');
        }
    ...
    private function _getPostResponseActionUrl(Mage_Sales_Model_Order $order)
    {
	 ...
	 if (Mage::getStoreConfig('cataloginventory/options/can_subtract', $order->getStoreId()) == true) {
        ...
        if (preg_match('/^(000\.400\.0|000\.400\.100)/', $resultJson['result']['code'])
            ...
            $pageName = 'checkout/onepage/success/';
            /*******/
            $register = Mage::getModel ('hyperpay/customerdata');
            if (isset($resultJson['registrationId'])) {
                ...
                if (Mage::getSingleton('customer/session')->isLoggedIn()) {
                    ...
                    $countdata = $read->fetchAll("select * from hyperpay_customer_data where cust_id = '" . $cust_id . "'");
			foreach ($countdata as $key => $value)
			{
                		if($value['ref_id'] == $resultJson['registrationId'])
					       $checkifexist = true;
			}
		    if(!$checkifexist)
		    {
                    	$register->setData("cust_id", $cust_id);
                    	$register->setData("ref_id", $resultJson['registrationId']);
                    	if (!empty($resultJson['card'])) {
                       	    $register->setData("last4digits", !empty($resultJson['card']['last4Digits']) ? $resultJson['card']['last4Digits'] : '');
                            $register->setData("holder", !empty($resultJson['card']['holder']) ? $resultJson['card']['holder'] : '');
                            $register->setData("expiry_month", !empty($resultJson['card']['expiryMonth']) ? $resultJson['card']['expiryMonth'] : '');
                            $register->setData("expiry_year", !empty($resultJson['card']['expiryYear']) ? $resultJson['card']['expiryYear'] : '');
                        }
                   if (empty($countdata))
                      $register->setData("default", "1");

                   try {
                     $insertId = $register->save()->getId();
                     Mage::log("Data register successfully inserted. Insert ID: " . $insertId, null, 'mylog.log');
                   } catch (Exception $e) {
